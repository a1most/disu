...
def kongzhiqingqiu_dnsscan(url,domain,header1,data,page=1):

	i=0
	while i<3:
		try:
			res = requests.post(url=url, headers=header1, data=data, timeout=(20,20), verify=False)
			...
			if res.status_code==200 and "404 Not Found" not in res.text:
				...
				logger.log('INFOR','		dnsscan 查询 {}第{}页错误，正在进行重试...'.format(domain,page))
				i=i+1
		except Exception as e:
			logger.log('INFOR','		dnsscan 查询 {}第{}页错误，正在进行重试...'.format(domain,page))
			i=i+1
	logger.log('ERROR','		dnsscan 查询 {}第{}页失败，该页返回空值。'.format(domain,page))
	return []
...
def dnsscan_get(domain):
	...
	data = r'ecmsfrom=36.36.211.23&show=%e5%b9%bf%e4%b8%9c%e7%9c%81&num=&classid=0&keywords={}'.format(domain)       # 数据包
	...
	try:
		...
		logger.log('INFOR',"		dnsscan发现{}域名{}条，共{}页。".format(domain,int(total_pipei1[0]),pages))
		#logger.log('INFOR','		dnsscan 正在从第1页中获取子域名信息..')
		if pages>1:
			for i in range(2,pages+1):
				url2="https://www.dnsscan.cn/dns.html?keywords={}&page={}".format(domain,i)
				#logger.log('INFOR','		dnsscan 正在从第{}页中获取子域名信息..'.format(i))
				try:
					resp2=kongzhiqingqiu_dnsscan(url2,domain,header1,data,i)
					domain_pipei2=re.findall(r'rel="nofollow" target="_blank">(.*?)</a></td>',resp2.text)
