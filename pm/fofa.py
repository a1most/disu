...
def fofa_get(domain,fofa_mail,fofa_key):
	...
	url=r"https://fofa.info/api/v1/search/all?email={}&key={}&qbase64={}&size={}".format(fofa_mail,fofa_key,domain,sizes)
	i=0
	while i<3:
		try:
			rsp=json.loads(requests.get(url=url, headers=header1, timeout=(10,10), verify=False).text)
			try:
				asb=rsp['results']
				#print (rsp)
				return asb
			...
			logger.log('ERROR','fofa 查询{}:{}'.format(domain,e.args))
			i=i+1