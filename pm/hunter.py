...
def kongzhiqingqiu(url,header1,domain):
	...
	while i<3:
		try:
			...
			if ret["code"]==200:
				...
				logger.log('INFOR',"		hunter查询{}错误，正在进行重试...".format(domain))
				i=i+1
		except:
			logger.log('INFOR',"		hunter查询{}错误，正在进行重试...".format(domain))
			i=i+1
		time.sleep(1)
	logger.log('ERROR','hunter 查询 {} 失败，即将返回空值。'.format(domain))
	return []	
...
def hunter_get(domain,hunter_key):
	...
	try:
		res=kongzhiqingqiu(url,header1,domain)
		ret = json.loads(res.text)
		#print (ret)
		if ret["code"] == 401:
			return []
		...
		logger.log('ERROR','hunter 查询 {} : {}'.format(domain, e.args))
		return qianxin_Results