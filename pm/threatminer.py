...
def kongzhiqingqiu_threatminer(url,header1,domain):
	...
	while i<3:
		try:
			...
			#print (res.text)
			if res.status_code==200 and ret['status_code']=='200':
				return ret
			else:
				logger.log('INFOR',"		threatminer查询{}错误，正在进行重试...".format(domain))
				i=i+1
		except Exception as e:
			logger.log('INFOR',"		threatminer查询{}错误，正在进行重试...".format(domain))
			i=i+1
	logger.log('ERROR','threatminer 查询 {}失败，即将返回空值。'.format(domain))
	#print ("返回空值")
	return []

def threatminer_get(domain):
	...
	try:
		resp=kongzhiqingqiu_threatminer(url,header1,domain)
		#print (resp['results'])
